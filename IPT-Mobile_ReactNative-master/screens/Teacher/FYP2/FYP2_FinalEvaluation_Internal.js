...
class FYP2_FinalEvaluation_Internal extends React.Component {
  ...
  async Submit() {
    const {FypID, leaderID, member1ID, member2ID, leaderMarks, member1Marks, member2Marks } = this.state;
  ...
  async getstatus() {
    ...
    await fetch('http://' + ip + '/api/fyp2get/Fyp2FinalInternalStatus?id=' + this.state.leaderID + ' ')
      .then(res => res.json())
      .then(users => {
